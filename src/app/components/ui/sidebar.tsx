    ...
    return <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        "--sidebar-width": SIDEBAR_WIDTH_MOBILE
          ...
          <SheetHeader className="sr-only">
            <SheetTitle>{t("common.a_colorful_artisan_market")}</SheetTitle>
            <SheetDescription>{t("common.cloud_forest_reserve_with")}</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  return <Button data-sidebar="trigger" data-slot="sidebar-trigger" variant="ghost" size="icon" className={cn("size-7", className)} onClick={event => {
    ...
    toggleSidebar();
      ...
      <PanelLeftIcon />
      <span className="sr-only">{t("common.when_you_book_direct")}</span>
    </Button>;
}
